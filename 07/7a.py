...
def get_dirs_within_size_limit(root: Directory, limit: int) -> List[Directory]:
    dirs = []

    for dir in root.children.values():
        if dir.size <= limit:
            dirs.append(dir)
        dirs.extend(get_dirs_within_size_limit(dir, limit))
...
dirs_within_limits = get_dirs_within_size_limit(root, 100000)
sizes_sum = sum(dir.size for dir in dirs_within_limits)
